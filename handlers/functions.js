  ...
  try {
    ...
    let queue_info_msg = await message.channel.messages.fetch(db.message_queue_info);
            ...
            if (!queue_info_msg) return message.channel.send(new MessageEmbed()).then(async msg => {
              message.edit(queueEmbed(client, player)).catch(e => console.log(e),console.log("Couldn't delete msg, this is for preventing a bug".gray));
              ...
              if (track_info_msg) track_info_msg.delete().catch(e => console.log(e),console.log("Couldn't delete msg, this is for preventing a bug".gray));
              return message.channel.send(new MessageEmbed()).then(msg => {
                msg.edit(SongEmbed(player.queue.current)).catch(e => console.log(e),console.log("Couldn't delete msg, this is for preventing a bug".gray));
    ...
    function SongEmbed(track) {
      ...
      let embed = new MessageEmbed()
        ...
        // embed.setTitle(`⌛️ ระยะเวลา:`)
        // embed.setDescription(`${createBarlul(player)}`)
        embed.addField("**เพลงโดย:**", `\`${track.author}\``, true)
        embed.addField(`**สถานะ**`, `\`${player.playing ? `${emoji.msg.resume}  กำลังเล่น` : `${emoji.msg.pause}  หยุดชั่วคราว`}\``, true)
        embed.addField(`**การร้องขอจาก:**`, `${track.requester}`, true)
        embed.setImage(`https://img.youtube.com/vi/${track.identifier}/mqdefault.jpg`)
        embed.setFooter(`คิว: ${Player.queue.size}  •  เสียง: ${Player.volume}%  •  วน: ${Player.get(`autoplay`) ? `✔️` : `❌`}  •  วน: ${Player.queueRepeat ? `✔️` : Player.trackRepeat ? `✔️` : `❌`}`, Player.queue.current.requester.displayAvatarURL({
          dynamic: true
        }))
      return embed;
    }
    function QueueEmbed(client, player) {
      ...
      const embed = new MessageEmbed().setAuthor(`คิวเพลงทั้งหมด`);
      const multiple = 15;
      const page = 1;
      const end = page * multiple;
      const start = end - multiple;
      const tracks = queue.slice(start, end);
      if (queue.current) embed.addField("**เพลงปัจจุบัน**", `${queue.current.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} [${track.isStream ? "ไลฟ์" : format(track.duration).split(" | ")[0]}]\nโดย: ${queue.current.requester}`);
      if (!tracks.length) embed.setDescription(`ไม่มีเพลงใน ${page > 1 ? `page ${page}` : "คิว"}.`);
      else embed.setDescription(tracks.map((track, i) => `**${start + ++i})** ${track.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} [${track.isStream ? "ไลฟ์" : format(track.duration).split(" | ")[0]}]\nโดย: ${track.requester}`).join("\n"));
      embed.setColor(ee.color);
      embed.setFooter(ee.footertext, ee.footericon);
      return embed;
    }
...
async function edit_request_message_queue_info(client, player) {
  try {
    s = [];
    const queue = player.queue;
    const embed = new MessageEmbed().setAuthor(`คิวเพลง`);
    const multiple = 15;
    const page = 1;
    const end = page * multiple;
    const start = end - multiple;
    const tracks = queue.slice(start, end);
    if (queue.current) embed.setDescription(`**0) กำลังเล่น** ${queue.current.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} ${queue.current.isStream ? "ไลฟ์" : format(queue.current.duration).split(" | ")[0]} เปิดเพลงโดย: ${queue.current.requester}`);
    if (!tracks.length) s.push(`No tracks in ${page > 1 ? `page ${page}` : "the queue"}.`);
    else embed.setDescription(`**0) กำลังเล่น** ${queue.current.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} ${queue.current.isStream ? "ไลฟ์" : format(queue.current.duration).split(" | ")[0]} เปิดเพลงโดย: ${queue.current.requester}\n`+tracks.map((track, i) => `**${start + ++i})** ${track.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} ${track.isStream ? "ไลฟ์" : format(track.duration).split(" | ")[0]} เปิดเพลงโดย: ${track.requester}`).join("\n"));
    embed.setColor(ee.color);
    embed.setFooter(ee.footertext, ee.footericon);
    embed;
    let message = player.get("message");
    ...
    track_info_msg.edit(oldembed.setFooter(`คิว: ${player.queue.size}  •  เสียง: ${player.volume}% •  วน: ${player.queueRepeat ? `✔️ คิว` : player.trackRepeat ? `✔️ เพลงเดียว` : `❌`} ${player.playing ? `${emoji.msg.resume}  กำลังเล่น` : `${emoji.msg.pause}  หยุดเล่น`}`, player.queue.current.requester.displayAvatarURL({
      dynamic: true
