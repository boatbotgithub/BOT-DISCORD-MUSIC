      ...
      client.manager
        .on("playerCreate", async (player) => {
            ...
            var embed = new MessageEmbed();
              embed.setTitle(`:thumbsup: **เข้าร่วม** \`${client.channels.cache.get(player.voiceChannel).name}\``)
              embed.setDescription(`**พิมคำสั่งจาก: ** <#${client.channels.cache.get(player.textChannel).id}>`)
        ...
        .on("trackStart", async (player, track) => {
          try {
            ...
            var embed = new MessageEmbed().setColor(ee.color)
			   embed.setTitle('ZELEN Music | กำลังเล่นเพลง')
            ...
            if(irc) {
              ...
              mi = setInterval(()=>{
                ...
                if(message && message.guild) {
                  message.channel.messages.fetch(client.setups.get(message.guild.id).message_track_info)
                  // .then(msg=>{
                  //   msg.edit(msg.embeds[0]).catch(e => console.log("Couldn't delete msg, this is for preventing a bug".gray));
                  // })
                    ...
                    switch (reactionemoji) {
                      ...
                      case String(emoji.react.show_current_track):
                        ...
                        return message.channel.send(new MessageEmbed()
                          ...
                          .addField(`ระยะเวลา: `, "`" + format(player.queue.current.duration) + "`", true)
                          .addField(`เพลงโดย: `, "`" + player.queue.current.author + "`", true)
                          .addField(`Queue length: `, `${player.queue.length} Songs`, true)
                          .addField(`Progress: `, createBar(player))
                          .setFooter(`การร้องขอจาก: ${player.queue.current.requester.tag}`, player.queue.current.requester.displayAvatarURL({
                            dynamic: true
                          }))
                        ).then(msg => {
                          if(msg && msg.deletable)   msg.delete({
            ...
            setTimeout(async () => {
              try {
                ...
                if (!player.queue || !player.queue.current) {
                  var embed = new MessageEmbed()
                    embed.setTitle("🔓 | เพลงจบแล้วบอทกำลังออกจากช่อง")
