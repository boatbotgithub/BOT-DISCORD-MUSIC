    ...
    try {
      ...
      if (!args[0])
        return message.channel.send(new MessageEmbed()
          ...
          .setFooter(ee.footertext, ee.footericon)
          .setTitle(`${emoji.msg.ERROR} | กรุณาเพิ่มวิธีการของคุณ`)
      ...
      if (args[0].toLowerCase() === `song` || args[0].toLowerCase() === `track` || args[0].toLowerCase() === `s` || args[0].toLowerCase() === `t`) {
        ...
        let embed = new MessageEmbed()
          .setTitle(`${emoji.msg.SUCCESS} | เปลี่ยนแทร็กลูปเป็น: ${player.trackRepeat ? `${emoji.msg.disabled} disabled` : `${emoji.msg.enabled} active`}`)
          .setColor(ee.color)
          .setFooter(ee.footertext, ee.footericon)
        //If Queue loop is enabled add embed info + disable it
        if (player.queueRepeat) {
          embed.setDescription(`และ **คิว** ย้ำ ได้ **${emoji.msg.disabled} disabled**`);
      ...
      //if input is queue
        ...
        //Create the Embed
        let embed = new MessageEmbed()
          .setTitle(`${emoji.msg.SUCCESS} | เปลี่ยน Queue loop เป็น: ${player.queueRepeat ? `${emoji.msg.disabled} disabled` : `${emoji.msg.enabled} active`}`)
          .setColor(ee.color)
          .setFooter(ee.footertext, ee.footericon)
